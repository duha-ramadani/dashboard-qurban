      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-slate-500 space-y-0.5">
            ...
            {hewan.keterangan && (
              <p className="text-slate-400 truncate">{hewan.keterangan}</p>
            )}
          </div>
        </div>
...
export default function HewanPage() {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    setHewan((prev) =>
      prev.map((h) => (h.id === hewanId ? { ...h, status: newStatus } : h))
    );
    await supabase.from("hewan").update({ status: newStatus }).eq("id", hewanId);
  }

  const filtered = hewan.filter(
    (h) => filterJenis === "semua" || h.jenis === filterJenis
  );

  const counts = {
    sapi: hewan.filter((h) => h.jenis === "sapi").length,
    kambing: hewan.filter((h) => h.jenis === "kambing").length,
    domba: hewan.filter((h) => h.jenis === "domba").length,
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        {(["semua", "sapi", "kambing", "domba"] as const).map((j) => (
          ...
          >
            {j === "semua"
              ? `Semua (${hewan.length})`
              : `${j.charAt(0).toUpperCase() + j.slice(1)} (${counts[j as JenisHewan]})`}
          </button>
        ))}
      </div>
        ...
        hewan.length === 0 ? (
          ...
          >
            <div className="flex gap-4 overflow-x-auto pb-4">
              ...
              <DroppableColumn
                status="sudah_disembelih"
                label="Sudah Disembelih"
                dotColor="bg-green-500"
                count={sudahDisembelih.length}
              >
                ...
                <div className="bg-white rounded-lg border-2 border-green-400 p-3 shadow-xl w-64 rotate-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-medium text-slate-800 text-sm">
                      {draggingHewan.nama_hewan ?? draggingHewan.jenis}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 mt-1">
                    {formatCurrency(draggingHewan.harga)}
                  </p>
                </div>
              )}
            </DragOverlay>
                    ...
                    <tr key={h.id} className="border-b border-slate-50 hover:bg-slate-50">
                      <td className="px-4 py-3 capitalize">
                        <span className="inline-flex items-center gap-1.5">
                          ...
                          <span className="text-slate-700 font-medium">{h.jenis}</span>
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-600">
                        {h.nama_hewan ?? <span className="text-slate-400 italic">—</span>}
                        {h.keterangan && (
                          <p className="text-xs text-slate-400">{h.keterangan}</p>
                        )}
                      </td>
                      <td className="px-4 py-3 text-slate-600">
                        {h.berat_kg ? `${h.berat_kg} kg` : "—"}
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <Badge
                          variant={
                            h.status === "belum_disembelih" ? "yellow" : "green"
                          }
                        >
                          {h.status === "belum_disembelih"
                            ? "Belum Disembelih"
                            : "Sudah Disembelih"}
                        </Badge>
                      </td>
      ...
      <Modal
        ...
        title={editId ? "Edit Hewan" : "Tambah Hewan"}
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleSave} disabled={saving || !form.harga}>
      ...
      >
        <p className="text-sm text-slate-600 mb-4">
          Yakin ingin menghapus data hewan ini? Data peserta yang terhubung akan
          terputus.
        </p>
