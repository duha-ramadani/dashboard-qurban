...
const emptyForm = {
  nama_penerima: "", alamat: "", no_hp: "",
  jenis_penerima: "fakir_miskin" as JenisPenerima,
  jumlah_paket: 1, berat_kg: "", hewan_id: "",
  tanggal_distribusi: new Date().toISOString().split("T")[0],
  catatan: "",
};

const jenisPenerimaMap: Record<JenisPenerima, { label: string; variant: "red" | "blue" | "green" | "gray" }> = {
  fakir_miskin: { label: "Fakir Miskin", variant: "red" },
  panitia: { label: "Panitia", variant: "blue" },
...
export default function DistribusiPage() {
  ...
  function openEdit(d: Distribusi) {
    setEditId(d.id);
    setForm({ nama_penerima: d.nama_penerima, alamat: d.alamat ?? "", no_hp: d.no_hp ?? "", jenis_penerima: d.jenis_penerima, jumlah_paket: d.jumlah_paket, berat_kg: d.berat_kg?.toString() ?? "", hewan_id: d.hewan_id ?? "", tanggal_distribusi: d.tanggal_distribusi, catatan: d.catatan ?? "" });
    setModalOpen(true);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const payload = { nama_penerima: form.nama_penerima, alamat: form.alamat || null, no_hp: form.no_hp || null, jenis_penerima: form.jenis_penerima, jumlah_paket: Number(form.jumlah_paket), berat_kg: form.berat_kg ? Number(form.berat_kg) : null, hewan_id: form.hewan_id || null, tanggal_distribusi: form.tanggal_distribusi, catatan: form.catatan || null };
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          { label: "Fakir Miskin", value: distribusi.filter((d) => d.jenis_penerima === "fakir_miskin").length },
        ].map(({ label, value }) => (
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={PackageOpen} title="Belum ada data distribusi" description="Tambahkan pencatatan distribusi daging qurban" action={<Button onClick={openAdd} size="sm"><Plus size={14} />Tambah Distribusi</Button>} />
        ) : (
                ...
                {distribusi.map((d) => {
                  ...
                  return (
                    <tr key={d.id} className="border-b border-slate-50 hover:bg-slate-50">
                      <td className="px-4 py-3 font-medium text-slate-800">{d.nama_penerima}{d.alamat && <p className="text-xs text-slate-400 font-normal">{d.alamat}</p>}</td>
                      ...
                      <td className="px-4 py-3 text-slate-600 capitalize">{d.hewan ? (d.hewan as Hewan).nama_hewan ?? (d.hewan as Hewan).jenis : "—"}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? "Edit Distribusi" : "Tambah Distribusi"}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Select label="Jenis Penerima" value={form.jenis_penerima} onChange={(e) => setForm({ ...form, jenis_penerima: e.target.value as JenisPenerima })} options={[{ value: "fakir_miskin", label: "Fakir Miskin" }, { value: "shohibul_qurban", label: "Shohibul Qurban" }, { value: "panitia", label: "Panitia" }, { value: "lainnya", label: "Lainnya" }]} />
          </div>
          ...
          <Select label="Dari Hewan" value={form.hewan_id} onChange={(e) => setForm({ ...form, hewan_id: e.target.value })} options={[{ value: "", label: "— Pilih hewan (opsional) —" }, ...hewan.map((h) => ({ value: h.id, label: `${h.nama_hewan ?? h.jenis} (${h.jenis})` }))]} />
