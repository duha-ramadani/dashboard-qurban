...
export default function LaporanPage() {
  ...
  function downloadDistribusi() {
    exportToExcel(
      distribusi.map((d) => ({
        ...
        "Berat (kg)": d.berat_kg ?? "",
        "Hewan Asal":
          (d.hewan as Hewan)?.nama_hewan ?? (d.hewan as Hewan)?.jenis ?? "",
  ...
  const hewanSudahDisembelih = hewan.filter(
    (h) => h.status === "sudah_disembelih"
  ).length;
  const totalPaketDistribusi = distribusi.reduce(
    (s, d) => s + d.jumlah_paket,
    0
  );
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Ringkasan dan export data qurban
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Ringkasan Kegiatan Qurban</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              {[
                {
                  ...
                  sub: `${pesertaLunas} lunas`,
                },
                {
                  label: "Total Hewan",
                  value: hewan.length,
                  sub: `${hewanSudahDisembelih} disembelih`,
                },
                {
                ...
                {
                  label: "Paket Distribusi",
                  value: totalPaketDistribusi,
                  sub: `${distribusi.length} penerima`,
                },
              ].map(({ label, value, sub }) => (
      ...
      {/* Rekap per jenis hewan */}
      <Card>
        <CardHeader>
          <CardTitle>Rekap Hewan per Jenis</CardTitle>
        </CardHeader>
