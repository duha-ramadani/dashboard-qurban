...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [pesertaRes, hewanRes, distribusiRes, settingsRes] =
          await Promise.all([
            supabase
              .from("peserta")
              .select("*")
        ...
        setStats({
          ...
          hewanBelumDisembelih: hewanData.filter(
            (h) => h.status === "belum_disembelih"
          ).length,
          hewanSudahDisembelih: hewanData.filter(
            (h) => h.status === "sudah_disembelih"
          ).length,
          totalPemasukan: pesertaData.reduce(
            (sum, p) => sum + (p.nominal_bayar ?? 0),
            0
          ),
          pesertaLunas: pesertaData.filter((p) => p.status_bayar === "lunas")
            .length,
          pesertaBelumLunas: pesertaData.filter(
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          subtitle={`${stats.sapiCount} sapi · ${stats.kambingCount} kambing · ${stats.dombaCount} domba`}
          icon={Beef}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Status Hewan</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-right px-6 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide">Bayar</th>
                ...
                {recentPeserta.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3 text-right">
                      <Badge
                        variant={
                          p.status_bayar === "lunas" ? "green" : "red"
                        }
                      >
